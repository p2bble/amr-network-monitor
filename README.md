...
### 자동 장애 진단 — AMR/MOXA 물류 창고 전용 4단계 심각도 (2026-04-22 전면 재설계)

일반 IT 기준이 아닌 MOXA AWK-1137C + H3C WA6320 + 폐쇄망 물류 창고 환경 기준.

| 단계 | 색상 | 의미 | 주요 조건 |
|------|------|------|-----------|
| 🔴 끊김 | 빨강 | AMR 실제 통신 단절 | 에이전트 무응답 / MOXA 단절 / AP·서버 ping 실패 |
| 🟠 위험 | 주황 | 수초~수십초 내 단절 위험 | 연속 3회 ping 실패 / 패킷손실 25%↑ / RSSI < -80 / TX Retry 30%↑ |
| 🟡 경고 | 노랑 | 품질 저하, 운영 가능 | Ping-pong 로밍 / 패킷손실 15%↑ / RSSI < -75 / TX Retry 20%↑ / Ping > 300ms |
| 🔵 주의 | 파랑 | 환경 불량, 운영 영향 없음 | SNMP 미수신 / Ping 100~300ms |
| 🟢 정상 | 초록 | 모든 지표 정상 | — |
...
| 구분 | 임계값 | 판정 | 비고 |
|------|--------|------|------|
| RSSI 경고 | -75 dBm 이하 | 🟡 경고 | 로밍 필요 구간 |
| RSSI 위험 | -80 dBm 이하 | 🟠 위험 | 실질 음영 근접 |
| Ping 주의 | 100 ms 이상 | 🔵 주의 | AMR 운영 영향 없음 |
| Ping 경고 | 300 ms 이상 | 🟡 경고 | 제어 응답 지연 시작 |
| Ping 위험 | 500 ms 이상 | 🟠 위험 | 제어 불안정 구간 |
| 패킷 손실 경고 | 15% 이상 | 🟡 경고 | 로밍 2회 이상 |
| 패킷 손실 위험 | 25% 이상 | 🟠 위험 | 지속적 손실 |
| TX Retry 경고 | 20% 이상 | 🟡 경고 | 채널 간섭 의심 |
| TX Retry 위험 | 30% 이상 | 🟠 위험 | 채널 혼잡 심각 |
| 연속 ping 실패 | 3회 이상 (15초) | 🟠 위험 | 로밍 순단(1~2회)과 구분 |
| 에이전트 무응답 | 30초 이상 | 🔴 끊김 | — |

> **로밍 순단 vs 실질 단절**: MOXA AWK-1137C는 비-802.11r 환경으로 AP 전환 시 62~414ms 단절 발생.
> ping 1~2회 실패 = 정상 로밍 순단. ping 3회 이상 연속 실패(15초) = 실질 단절로 구분.

**클라이언트 진단** (대시보드 열람 중에만 동작) — 위 4단계 심각도 기준 적용
...
## 임계값 기준 (AMR/MOXA 물류 창고 환경 기준 — 2026-04-22 재설계)

> 일반 IT 기준(ping 80ms=경고, RSSI -72=경고)과 다름. AMR 운영 영향 기반.

| 구분 | 임계값 | 판정 |
|------|--------|------|
| RSSI 경고 | -75 dBm 이하 | 🟡 경고 |
| RSSI 위험 | -80 dBm 이하 | 🟠 위험 |
| Ping 주의 | 100 ms 이상 | 🔵 주의 (운영 영향 없음) |
| Ping 경고 | 300 ms 이상 | 🟡 경고 |
| Ping 위험 | 500 ms 이상 | 🟠 위험 |
| 패킷 손실 경고 | 15% 이상 | 🟡 경고 |
| 패킷 손실 위험 | 25% 이상 | 🟠 위험 |
| 연속 ping 실패 | 3회(15초) 이상 | 🟠 위험 |
| TX Retry 경고 | 20% 이상 | 🟡 경고 |
| TX Retry 위험 | 30% 이상 | 🟠 위험 |
| Sticky Client | RSSI < -75 + 30초 로밍 없음 | 🟡 경고 |
| 에이전트 무응답 | 30초 이상 | 🔴 끊김 |
...
| AP-05 | 192.168.145.35 | — | — | 미운영 (DISABLED) |
...
| AP-10 | 192.168.145.40 | — | — | 미운영 (DISABLED) |
| AP-11 | 192.168.145.41 | Ch 36 | **5dBm** | 밀집 구간 저전력 |
| AP-12 | 192.168.145.42 | Ch 149 | **7dBm** | radio shutdown (SHUTDOWN) — WiFi OFF |
| AP-13 | 192.168.145.43 | **Ch 161** | **6dBm** | 2026-04-20 활성화 |
| AP-14 | 192.168.145.44 | Ch 153 | **7dBm** | radio shutdown (SHUTDOWN) — WiFi OFF |
| AP-15 | 192.168.145.45 | Ch 40 | **7dBm** | radio shutdown (SHUTDOWN) — WiFi OFF |
...
| 날짜 | 내용 |
|------|------|
| 2026-04-22 | 앱 이름 변경: 'AMR Network Dashboard' → '세방전지 AMR 통신 관제' |
| 2026-04-22 | `DiagnosisEngine` 전면 재설계: 일반 IT 기준 → AMR/MOXA 물류 창고 전용 4단계 심각도 (🔴끊김/🟠위험/🟡경고/🔵주의) |
| 2026-04-22 | `amr_unified_agent.py`: `ping_fail_streak` (연속 실패 횟수), `sustained_loss` (3회+=실질단절) 필드 추가 |
| 2026-04-22 | `amr_unified_agent.py`: `pingpong` / `pingpong_pair` ping-pong 로밍 감지 (패턴 기반, 5분 내 3회 이상) |
| 2026-04-22 | `ap_admin_config.json`: AP-12 `ACTIVE` → `SHUTDOWN` (H3C CLI `shutdown` 확인), DISABLED/SHUTDOWN 구분 적용 |
| 2026-04-22 | Flutter 대시보드: RSSI 등급 기준 재조정 (-65우수/-72양호/-78경고/-83위험) |
| 2026-04-22 | Flutter 대시보드: Ping-pong 로밍 감지 (대시보드 자체 fallback + 에이전트 수신값 병행) |
| 2026-04-22 | Flutter 대시보드: 통신 현황 보고서 CSV 내보내기 기능 추가 (기간/심각도/로봇 필터, UTF-8 BOM) |
